...
export class TriageListComponent {
  ...
  triages = signal(
    [...this.mockData.getTriages()].sort((a, b) => {
      const order: Record<string, number> = { CRITIQUE: 0, URGENT: 1, SEMI_URGENT: 2, NON_URGENT: 3, NORMAL: 4 };
      return order[a.niveauUrgence] - order[b.niveauUrgence];
    })
  );

  countByNiveau(n: NiveauUrgence): number {
    return this.triages().filter(t => t.niveauUrgence === n).length;
  }
